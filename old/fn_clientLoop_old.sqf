...
AIC_clientPFH = [{
    ...
    private _candidateSet = createHashMapFromArray (_candidates apply { [_x call BIS_fnc_netId, true] });

    // --- Every tick: reveal from the COMMITTED set (left pool / closed inside safe radius) ---
    {
        if (isNull _x) then { continue };
        if !(_candidateSet getOrDefault [_x call BIS_fnc_netId, false]) then { _x hideObject false; continue };
    ...
    if (AIC_clientSweepHidden isNotEqualTo []) then {
        ...
        {
            if (isNull _x) then { continue };
            if (!(_candidateSet getOrDefault [_x call BIS_fnc_netId, false]) || { (_x distance player) <= AIC_clientSafeRadius }) then {
                _x hideObject false;
            } else {
    ...
    for "_i" from AIC_clientCursor to (_end - 1) do {
        ...
        if ((_unit distance player) <= AIC_clientSafeRadius) then {
            ...
            private _hits = lineIntersectsSurfaces [_playerEyePos, _unitEyePos, vehicle player, _unit, true, 1];
