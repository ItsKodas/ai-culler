...
- Fixed AIC status panel hide/show being unreliable when pressing backspace in Zeus — replaced the manual KeyDown toggle with `displayAddEventHandler ["Hide"]` and `["Show"]` so the panel follows Zeus's own display state directly instead of trying to track it independently
